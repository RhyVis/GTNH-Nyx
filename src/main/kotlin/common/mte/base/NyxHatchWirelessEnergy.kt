...
class NyxHatchWirelessEnergy : MTEHatchEnergyMulti {
    ...
    private val vol: Long
        get() = V[mTier.toInt()]

    override fun getTexturesActive(aBaseTexture: ITexture?): Array<ITexture?> = arrayOf(aBaseTexture, textureOverlay[mTier.toInt() + 1])

    override fun getTexturesInactive(aBaseTexture: ITexture?): Array<ITexture?> = arrayOf(aBaseTexture, textureOverlay[mTier.toInt() + 1])
    ...
    override fun onPreTick(
        ...
        if (aBaseMetaTileEntity.isServerSide &&
            aTick % helper.actualTicksBetweenEnergyAddition == 0L
        ) {
            tryFetchingEnergy()
        }
    }
...
class NyxHatchWirelessDynamo : MTEHatchDynamoMulti {
    ...
    override fun onWireCutterRightClick(
        ...
        if (tryStoringEnergy()) {
            GTUtility.sendChatToPlayer(
                aPlayer,
                "Instantly transferred energy to the network.",
            )
        }
        return false
    }
    ...
    private fun tryStoringEnergy(): Boolean {
        if (euVar < 0) euVar = 0
        if (euVar == 0L || !WirelessNetworkManager.addEUToGlobalEnergyMap(ownerUUID, euVar)) {
            return false
        }
        euVar = 0
        return true
...
private class TransferHelper(
    ...
    private val transferPerOpt: BigInteger =
        BigInteger.valueOf(amp * V[tier.toInt()]) *
            BigInteger.valueOf(WirelessNetworkManager.ticks_between_energy_addition)
    ...
    val transferPerOptLong: Long =
        if (overflowDivisor > 1) {
            (
                transferPerOpt /
                    BigInteger.valueOf((overflowDivisor * precisionMultiplier * 2).toLong()) *
                    BigInteger.valueOf(precisionMultiplier)
            ).toLong()
        } else {
    ...
    val actualTicksBetweenEnergyAddition =
        if (overflowDivisor > 1) {
            (WirelessNetworkManager.ticks_between_energy_addition / (overflowDivisor * 2)).toLong()
        } else {
...
private fun mapTextureSet(amp: Int) =
    when (amp) {
        4 -> Textures.BlockIcons.OVERLAYS_ENERGY_ON_WIRELESS_4A
        16 -> Textures.BlockIcons.OVERLAYS_ENERGY_ON_WIRELESS_16A
        64 -> Textures.BlockIcons.OVERLAYS_ENERGY_ON_WIRELESS_64A
        else -> Textures.BlockIcons.OVERLAYS_ENERGY_ON_WIRELESS_LASER
    }
