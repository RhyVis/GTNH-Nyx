...
class NyxGeneratedMetaItem(
    ...
    override fun addAdditionalToolTips(
        ...
        if (material.extraTooltip.isNotEmpty()) {
            if (isShiftKeyDown()) {
                material.extraTooltip.forEach { aList.add(it) }
            } else {
                aList.add("按下 ${BOLD}${WHITE}Shift${RESET}$GRAY 键以查看更多信息")
            }
        }
    }
    ...
    override fun getIconFromDamage(aMetaData: Int): IIcon? =
        if (aMetaData < 0 || aMetaData >= MaterialSet.size) {
            null
        } else if (MaterialMap[aMetaData.toShort()] == null) {
            null
        } else {
