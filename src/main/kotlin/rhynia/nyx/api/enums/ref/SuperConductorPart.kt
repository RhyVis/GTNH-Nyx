...
enum class SuperConductorPart(private val production: Materials, private val raw: Materials) {
  ...
  fun getSxEqualFluid(amount: Int, raw: Boolean = false): FluidStack =
      if (raw) NyxMaterials.SuperconductorFlux.getLiquid(2.0.pow(ordinal).toInt() * amount * 4)
      else NyxMaterials.SuperconductorFlux.getLiquid(2.0.pow(ordinal).toInt() * amount)
}
