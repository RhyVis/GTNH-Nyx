...
object ItemUtil {
  ...
  fun isAstralInfinityGem(itemStack: ItemStack?): Boolean {
    ...
    return itemStack.isItemEqual(CelItemList.AstriumInfinityGem.get(1))
  }

  fun isAstralInfinityComplex(itemStack: ItemStack?): Boolean {
    if (itemStack == null) return false
    return itemStack.isItemEqual(CelItemList.AstriumInfinityComplex.get(1))
  }

  fun isAstralInfinityGauge(itemStack: ItemStack?): Boolean {
    if (itemStack == null) return false
    return itemStack.isItemEqual(CelItemList.AstriumInfinityGauge.get(1))
  }

  fun isCalibration(itemStack: ItemStack?): Boolean {
    if (itemStack == null) return false
    return itemStack.isItemEqual(CelItemList.Calibration.get(1))
  }
}
