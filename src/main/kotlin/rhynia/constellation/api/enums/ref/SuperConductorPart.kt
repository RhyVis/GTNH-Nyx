...
enum class SuperConductorPart(private val production: Materials, private val raw: Materials) {
  ...
  fun getSxEqualFluid(amount: Int, raw: Boolean = false): FluidStack =
      if (raw) CelMaterials.SuperconductorFlux.getLiquid(2.0.pow(ordinal).toInt() * amount * 4)
      else CelMaterials.SuperconductorFlux.getLiquid(2.0.pow(ordinal).toInt() * amount)
}
