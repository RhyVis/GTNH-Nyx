...
import vis.rhynia.nova.api.process.util.NovaItemId;
import vis.rhynia.nova.api.util.ItemUtil;
import vis.rhynia.nova.api.util.MathUtil;

public class NovaParallelHelper extends ParallelHelper {
     ...
     */
    private final MaxParallelCalculator maxParallelCalculator = NovaParallelHelper::maxParallelCalculatedByInputs;
     ...
     */
    private final InputConsumer inputConsumer = NovaParallelHelper::consumeInput;
     ...
     */
    public @NotNull ParallelHelper setMachine(IVoidable machine) {
     ...
     */
    public @NotNull ParallelHelper setMachine(IVoidable machine, boolean protectExcessItem,
     ...
     */
    public @NotNull ParallelHelper setRecipe(@Nonnull GTRecipe aRecipe) {
        recipe = Objects.requireNonNull(aRecipe);
        return this;
    }

    public @NotNull ParallelHelper setRecipeLocked(IRecipeLockable singleRecipeMachine, boolean isRecipeLocked) {
     ...
     */
    public @NotNull ParallelHelper setItemInputs(ItemStack... aItemInputs) {
     ...
     */
    public @NotNull ParallelHelper setFluidInputs(FluidStack... aFluidInputs) {
     ...
     */
    public @NotNull ParallelHelper setAvailableEUt(long aAvailableEUt) {
    ...
    public @NotNull ParallelHelper setCalculator(OverclockCalculator calculator) {
     ...
     */
    public @NotNull ParallelHelper setConsumption(boolean consume) {
     ...
     */
    public @NotNull ParallelHelper setMaxParallel(int maxParallel) {
     ...
     */
    public @NotNull ParallelHelper enableBatchMode(int batchModifier) {
     ...
     */
    public @NotNull ParallelHelper setOutputCalculation(boolean calculateOutputs) {
     ...
     * array
     */
    public @NotNull ParallelHelper setCustomItemOutputCalculation(Function<Integer, ItemStack[]> custom) {
     ...
     * array
     */
    public @NotNull ParallelHelper setCustomFluidOutputCalculation(Function<Integer, FluidStack[]> custom) {
    ...
    /**
     * Finishes the ParallelHelper. Anything changed after this will not affect anything
     */
    public @NotNull ParallelHelper build() {
        if (built) {
    ...
    protected void determineParallel() {
        ...
        if (tickTimeAfterOC < 1) {
            maxParallel = MathUtil.INSTANCE.safeInt((long) (maxParallel / tickTimeAfterOC), 1);
        }

        int maxParallelBeforeBatchMode = maxParallel;
        if (batchMode) {
            maxParallel = MathUtil.INSTANCE.safeInt((long) maxParallel * batchModifier, 1);
        }
        ...
        for (int i = 0; i < recipe.mOutputs.length; i++) {
            ...
            if (outputChance < 10000) {
                ...
                while (outputs >= Integer.MAX_VALUE) {
                    toOutput.add(ItemUtil.INSTANCE.setStackSize(origin.copy(), Integer.MAX_VALUE));
                    outputs -= Integer.MAX_VALUE;
                }

                if (outputs > 0) {
                    toOutput.add(ItemUtil.INSTANCE.setStackSize(origin.copy(), (int) outputs));
                }
    ...
    public static double maxParallelCalculatedByInputs(GTRecipe recipe, int maxParallel, FluidStack[] aFluidInputs,
        ...
        if (recipe.mInputs.length > 0) {
            Map<NovaItemId, Long> itemCost = new HashMap<>();
            if (isNBTSensitive) {
                for (ItemStack recipeItemCost : recipe.mInputs) {
                    // for non-consumed input
                    if (recipeItemCost.stackSize == 0) continue;
                    itemCost.merge(
                        (NovaItemId) NovaItemId.create(recipeItemCost),
                ...
                for (ItemStack recipeItemCost : recipe.mInputs) {
                    ...
                    if (recipeItemCost.stackSize == 0) continue;
                    itemCost.merge(
                        (NovaItemId) NovaItemId.createWithoutNBT(recipeItemCost),
                        (long) recipeItemCost.stackSize,
                        Long::sum);
                }
            }

            nextRecipeItemCost: for (Map.Entry<NovaItemId, Long> itemID : itemCost.entrySet()) {
